...
router.get('/live-stats', async (req: Request, res: Response) => {
  try {
    ...
    const uniqueParticipants = await prisma.entry.groupBy({
      by: ['userId'],
      where: { drawId: activeDraw.id },
    });
    ...
    const tokensCount = await prisma.digitalToken.count({
      where: { status: 'ACTIVE' },
    });
    const coinsCount = await prisma.physicalCoin.count({
...
router.get('/verify-fairness/:number', async (req: Request, res: Response) => {
  const { number } = req.params;
  try {
    const luckyNumber = await prisma.luckyNumber.findUnique({
      where: { number },
      include: {
        user: {
          select: {
